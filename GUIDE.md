...
> 需要管理员权限时会先尝试 Touch ID，失败或不支持时自动回退为密码输入。
...
Mole 通过 `sudo` 请求管理员权限，默认提示输入密码。若你已经为 `sudo` 启用了 Touch ID，同一个提示可以直接按指纹，失败后再输入密码。
...
3. 保存退出（`Ctrl+O` 回车，再 `Ctrl+X`）。以后在终端运行 `sudo` 命令时就会先提示 Touch ID，仍可随时改为输入密码。
